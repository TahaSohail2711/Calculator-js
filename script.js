...
let varName = document.getElementById('var-name');
let varVal = document.getElementById('var-val');
let saveStart = 0;
let saveEnd = 0;
let Variables = {
...
let append = (val) => {
    if (input != null) {
        ...
        const newValue = currentValue.substring(0, Number(startPos)) +
            val +
        ...
        if (val.startsWith('Variables[')) {
            ...
            input.setSelectionRange(cursorPos + varNameLength, cursorPos + varNameLength);
        }
        else {
